...
check('Admin: smagsprofil fulgte med fra biblioteket', (await f1.locator('input[name=duft]:checked').evaluateAll((els) => els.map((e) => e.value))).join() === 'Kraftig' && (await f1.locator('input[name=aromaer]:checked').evaluateAll((els) => els.map((e) => e.value))).join() === 'Banan');
...
check('Afsløring: admins smagsprofil vises (duft, smag, aromaer)', after.includes('Administratorens smagsprofil') && after.includes('Kraftig') && after.includes('Lang eftersmag') && after.includes('Banan'));
